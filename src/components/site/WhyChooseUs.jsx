        ...
        <div className="mt-14 grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {benefits.map((b, i) => (
            <Reveal key={b.title} delay={i * 0.07}>
              <div className="group h-full rounded-2xl border border-white/10 bg-white/5 p-7 transition-all hover:-translate-y-1 hover:shadow-lg hover:border-electric/30">
                <span className="flex h-12 w-12 items-center justify-center rounded-xl bg-white/10 text-electric shadow-sm transition-colors group-hover:bg-electric group-hover:text-white">
                  <b.icon className="h-6 w-6" />
                </span>
                <h3 className="mt-5 font-heading text-lg font-semibold text-white">{b.title}</h3>
